...
<p>L'espèce <i>castellanus</i> est une espèce de
 <xsl:value-of select="count(//espece[text()='castellanus'])"/> nuages différents.</p>
</body>
</html>
</xsl:template>
...
<xsl:value-of select="concat(translate(substring($name,1,1),'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'),substring-after($name,substring($name,1,1)))"/>
</xsl:template>
</xsl:stylesheet>
